...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  if (locale !== "de" && locale !== "en") return <LocalizedEditorialShell locale={locale} routePath="/vollprofil-buch" />;
  const page = getPillarPage("profilBuch", locale)!;
  return (
    <>
      <JsonLd data={[faqSchema(page.faq), articleSchema({ headline: page.seoTitle, description: page.seoDescription, locale, url: `${siteUrl}/${locale}${paths.profilBuch}/`, about: "Hermetia Vollprofil-Buch", image: `${siteUrl}${page.image}` })]} />
