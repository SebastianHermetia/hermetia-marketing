  ...
  const labels =
    locale === "de"
      ...
      : locale === "en"
        ...
        : { primary: ui.startFree, secondary: ui.premium ?? ui.learnMore };
  const safeTitle = locale === "de" || locale === "en" ? title : ui.ctaTitle;
  const safeText = locale === "de" || locale === "en" ? text : ui.ctaText;
  return (
    <div className="rounded-card bg-gradient-to-br from-aubergine to-pflaume px-7 py-10 text-center text-white">
      <h2 className="mx-auto max-w-[720px] text-[clamp(25px,3.5vw,34px)] text-white">{safeTitle}</h2>
      <p className="mx-auto my-4 max-w-[560px] text-[#e9dcf2]">{safeText}</p>
